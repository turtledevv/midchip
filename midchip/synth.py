...
from . import ui
from .waveforms import generate as gen_wave, generate_drum
from .chips import ChipProfile
...
def _envelope(w: np.ndarray, attack: float, release: float) -> np.ndarray:
    n = len(w)
...
def _vibrato_depth_semitones(freq: float) -> float:
    ...
    return VIBRATO_DEPTH_SEMITONES * scale
...
def _pitch_ratio_curve(
    ...
    if vibrato:
        ...
        depth = _vibrato_depth_semitones(freq)
        semitone_shift = semitone_shift + (
            depth * ramp * np.sin(2 * np.pi * VIBRATO_RATE_HZ * t)
        )
...
def render_note(
    ...
    if drum_category is None and unison_voices > 1 and unison_detune_cents > 0:
        ...
        for i in range(unison_voices):
            ...
            acc = acc + render_note(
                ...
                vibrato=vibrato, unison_voices=1,
            )
        w = acc / unison_voices
        if rms_fix:
    ...
    if drum_category is not None:
        w = generate_drum(drum_category, t)
    ...
    if has_bend or vibrato:
        ratio = _pitch_ratio_curve(t, note_start_time, bend_times, bend_semitones, vibrato, freq=freq)
        t_gen = np.cumsum(ratio) / SAMPLE_RATE

    w = gen_wave(wave_type, t_gen, freq, triangle_steps=triangle_steps)
    w = _envelope(w, attack, release)
...
def _reverb_channel(x: np.ndarray, rate: int, offset_ms: float, chorus_phase: float = 0.0) -> np.ndarray:
    wet = np.zeros_like(x)
    for delay_ms in REVERB_COMB_DELAYS_MS:
        d = max(1, int(round((delay_ms + offset_ms) * rate / 1000.0)))
        wet = wet + _comb_filter(x, d, REVERB_DECAY)
...
def apply_reverb(out: np.ndarray, rate: int = SAMPLE_RATE) -> np.ndarray:
    ...
    if out.ndim == 2:
        ...
        wet_l = _reverb_channel(out[:, 0], rate, -spread, chorus_phase=0.0)
        wet_r = _reverb_channel(out[:, 1], rate, spread, chorus_phase=math.pi / 2.0)
        wet = np.stack([wet_l, wet_r], axis=1)
    else:
        wet = _reverb_channel(out, rate, 0.0)

    return out * (1.0 - REVERB_MIX) + wet * REVERB_MIX
...
def _synthesize_note(
    ...
    wave = render_note(
        midi_to_freq(iv.note), dur, iv.wave_type, vel,
...
def _render_chunk_worker(payload):
    (chunk, chip, note_attack, note_release, bend_arrays, cc_state,
     vibrato, rms_fix, unison_voices, unison_detune_cents) = payload
    return [
        _synthesize_note(
...
def render_audio(
    ...
    max_release = max(RELEASE, max(DRUM_RELEASE.values()))
    ...
    note_attack = chip.attack if chip else ATTACK
    note_release = chip.release if chip else RELEASE
